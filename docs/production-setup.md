...
- A machine to run the XAGON node with the following requirements:
...
- A L1 node: we recommend using geth, but what it's actually needed is access to a JSON RPC interface for the L1 network (Goerli for XAGON testnet, Ethereum mainnet for XAGON mainnet)
